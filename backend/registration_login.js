...
// Funzione per convertire una data da gg-mm-aaaa a aaaa-mm-gg
...
app.post('/register', [
  ...
  try {
    // Converti la data di nascita e la data di scadenza dell'assicurazione nel formato corretto
    const formattedBirthDate = convertToISODate(birth_date);
    const formattedExpiryDate = professional_insurance_expiry_date ? convertToISODate(professional_insurance_expiry_date) : null;
    ...
    if (role === 0) {
      ...
      result = await pool.query(
        ...
        [email, hashedPassword, first_name, last_name, formattedBirthDate]
      );
    } else if (role === 1) {
      ...
      result = await pool.query(
        ...
        [email, hashedPassword, first_name, last_name, formattedBirthDate, address, vat_number, professional_insurance_number, formattedExpiryDate, iban, professional_association_registration]
      );
    }
